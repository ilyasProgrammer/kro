...
class Task(models.Model):
    ...
    amount = fields.Float(u'Бюджет', track_visibility='onchange', copy=False)
    ...
    mark_state = fields.Integer(string=u'Оценка статуса', track_visibility='onchange', group_operator='avg', copy=False)
    mark_result = fields.Integer(string=u'Оценка результата', track_visibility='onchange', group_operator='avg', copy=False)
    ...
    def _get_responsible(self):
        ...
        if self.state not in ['agreement', 'plan']:
            if self.got_approver:
                if not self.approved_by_executor or not self.approved_by_predicator or not self.approved_by_approver:
                    self.sudo().write({'state': 'agreement'})
            else:
                if not self.approved_by_executor or not self.approved_by_predicator:
                    self.sudo().write({'state': 'agreement'})
